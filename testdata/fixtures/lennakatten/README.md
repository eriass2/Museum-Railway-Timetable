...
**Traffic demo (not in CSV):** After import, `MRT_lennakatten_apply_traffic_demo_data()` seeds trafikmeddelanden (B) and tur-avvikelser (A) for manual/PHPUnit testing of the UL-like disruption feed — see `inc/import/lennakatten/traffic-demo-data.php`. Reference date **2026-06-06**: glassrea + inställda turer 71/97, ersättningsbuss tur 75, försenad buss B3; upcoming baninfo Jul–Aug and inställd tur 71 on **2026-07-04**. First line of each notice becomes `summary` in the feed API.
