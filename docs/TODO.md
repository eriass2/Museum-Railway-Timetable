...
**Källa:** kodgranskning 2026-06-13 (`frontend/vue/src/`). Projektet är i stort välstrukturerat (Composition API, strict TS, feature-mappar); punkterna nedan är inkonsekvenser och luckor, inte grundläggande anti-mönster.

| ID | Punkt | Prioritet | Insats |
|----|-------|-----------|--------|
| V1 | **Admin — enhetlig felhantering vid mutationer.** `adminFetch` kastar `AdminRestError`; många `use*Page`-anrop saknar `try/catch` och visar inget fel till användaren (t.ex. `useTrainTypesPage`, `useTrafficNoticesPage`, delar av `useStationsRoutesPage`, `useTimetableEditorPage`). Standardisera på mönstret i `useSettingsPage` (`adminErrorMessage` / `showSaveNotice`). | Hög | Medel |
| V2 | **ESLint i Vue-gate.** `vue-check` kör typecheck + test + build men ingen lint. Lägg till `eslint-plugin-vue` + `@typescript-eslint` och `npm run lint` i `check`-skriptet. | Medel | Medel |
| V3 | **Dela stora composables.** `useTimetableEditorPage.ts` (~410 rader) och `useStationsRoutesPage.ts` (~410 rader) — dela per flik/domän enligt befintlig feature-mappstruktur. | Medel | Stor |
| V4 | **A11y — feedback-dialog.** `WizardFeedbackWidget.vue`: `aria-labelledby`, stäng-knapp med `aria-label`, Escape + fokusfälla (som `AdminConfirmDialog`). | Medel | Liten |
| V5 | **A11y — combobox.** `MrtCombobox.vue`: tangentbordsnavigation, `aria-activedescendant`, listbox-mönster enligt WAI-ARIA. | Medel | Medel |
| V6 | **A11y — admin i e2e.** Utöka `e2e/a11y-smoke.spec.ts` med minst en admin-route (dashboard). | Låg | Liten |
| V7 | **Stabila `:key` i wizard.** `WizardTripStep.vue` använder `:key="idx"` — byt till stabil id/hash om listan kan ändra ordning. | Låg | Liten |
| V8 | **i18n — kvarvarande hårdkodade strängar.** T.ex. `<legend>Typ</legend>` i `WizardFeedbackWidget`, default-fallbacks i `MrtAsyncState` / `MrtCalendarNav` — route via `cfgStr`/`adminStr` där PHP-config finns. | Låg | Liten |
| V9 | **Admin scaffold — DRY.** Upprepat list/edit/create + dirty guard i flera `use*Page`; överväg gemensam `useAdminListEditorPage<T>()` när V1 är klart. | Låg | Medel |

**Rekommenderad ordning:** V1 → V2 → V4 → V3 (V5–V9 vid behov eller feature-touch).
