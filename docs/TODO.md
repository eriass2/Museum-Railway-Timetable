...
**Plan:** [CSS_FOLLOWUP_PLAN.md](CSS_FOLLOWUP_PLAN.md) — C6–C9 + E0 klart (2026-06-13); E1-fixar mergade, E2E-gate återstår.
...
| R-J3 | Förenkla behov-flaggor i API (`behov_hint`) | öppen |
| R-J1 | Ca-semantik wizard vs Turvy (beslut D23) + `STOP_TIME_CA.md` | öppen |
| R-J5 | Timeline E2E/screenshot-regression | ✅ |
| R-T1 | Ta bort TS legacy body-fallback när API alltid sätter `detail_intro` | öppen — se **TF-A** / **TF-F2** |
...
| TF-0.2 | `assets/mrt-traffic-info-tokens.css` (`--mrt-tf-*`) | Frontend | ✅ fil, ej inkopplad |
| TF-0.3 | UL-referensbild `mockups/ul-trafikinfo-reference.png` (UL + målbild) | Produkt | öppen |
| TF-0.4 | Side-by-side UL vs snapshot i token-doc | Design | öppen |

### Fas A — Data & presentation (PHP)

| ID | Punkt | Ansvar | Status |
|----|-------|--------|--------|
| TF-A1 | Fält `summary` (korall-text, utan datum) | Backend | öppen |
| TF-A2 | Fält `validity_label` («Gäller …», separat från rubrik) | Backend | öppen |
| TF-A3 | Fält `line_label`, `severity`, `category_key`, `category_label`, `icon_key` | Backend | öppen |
| TF-A4 | Sluta dubblera datum i `headline` när datum visas separat | Backend | öppen |
| TF-A5 | Svenskt datum i `validity_label` («14 juni», inte `2026-09-03`) | Backend | öppen |
| TF-A6 | Avvikelse: `summary` = händelse, `line_label` = tågnummer (inte allt i headline) | Backend | öppen |
| TF-A7 | i18n: «Aktuellt trafikläge», «Planerade avvikelser», giltighet-mall | Backend | öppen |

### Fas B — API `panels` (hierarki)

| ID | Punkt | Ansvar | Status |
|----|-------|--------|--------|
| TF-B1 | `panels[]` med `categories[]` + `counts` (info/warning) | Backend | öppen |
| TF-B2 | `MRT_disruption_feed_build_panels()` — gruppering Tåg/Buss/Information | Backend | öppen |
| TF-B3 | TypeScript-typer i `disruptionFeed.ts` | Frontend | öppen |
| TF-B4 | Legacy `ongoing`/`upcoming` kvar tills noscript migrerat | Backend | öppen |
...
| ID | Punkt | Ansvar | Status |
|----|-------|--------|--------|
| TF-C1 | `traffic-info-layout.css` — BEM layout, inga färger | Frontend | öppen |
| TF-C2 | Importera `mrt-traffic-info-tokens.css` i trafikinfo-chunk | Frontend | öppen |
| TF-C3 | `MrtTfPanels` → `Panel` → `CategoryRow` → `AlertList` → `AlertCard` | Frontend | öppen |
| TF-C4 | Primitiver: `MrtTfCountBadge`, `MrtTfLineBadge`, ikoner (klocka, kalender, varning) | Frontend | öppen |
| TF-C5 | `MrtTfCountBadge` (UL pill); `MrtInfoMark` finns för wizard — **ej** trafikbadge än | Frontend | öppen |
| TF-C6 | State: `expandedCategoryKey`; valfritt `expandedAlertId` | Frontend | öppen |
| TF-C7 | Deprecate: `MrtDisruptionFeedSections`, `MrtDisruptionFeedItemCard`, «Mer information» | Frontend | öppen |
| TF-C8 | Edge: tom `ongoing`, bara `upcoming`; mobil radbrytning giltighet | Frontend | öppen |
| TF-C9 | DoD: `grep style=` i `traffic-notices/` → tomt | Frontend | öppen |
| TF-C10 | Uppdatera [VUE_UI_COMPONENTS.md](VUE_UI_COMPONENTS.md) med `MrtTf*` | Frontend | öppen |

### Fas D — Noscript (shortcode)

| ID | Punkt | Ansvar | Status |
|----|-------|--------|--------|
| TF-D1 | `shortcode.php` — samma BEM `.mrt-tf-*` som Vue | Backend | öppen |
| TF-D2 | Inga `style=` i noscript-HTML | Backend | öppen |
| TF-D3 | Ta bort legacy `mrt-traffic-notices__feed-item` i noscript | Backend | öppen |

### Fas E — Admin

| ID | Punkt | Ansvar | Status |
|----|-------|--------|--------|
| TF-E1 | `TrafficNoticesFeedPreview` — samma `MrtTf*` som publik vy | Admin FE | öppen |
| TF-E2 | Operatörguide i admin-hjälp (första rad = meddelande, datum automatiskt) | Produkt + Admin | öppen |

### Fas F — Test

| ID | Punkt | Ansvar | Status |
|----|-------|--------|--------|
| TF-F1 | PHPUnit: `panels`, counts, `summary`/`validity_label` | Test | öppen |
| TF-F2 | Vitest: badge-räkning, severity, gruppering; städa **R-T1** legacy body | Test | öppen |
| TF-F3 | E2E: expand kategori → alert synlig (UL-flöde) | Test | öppen |
| TF-F4 | Playwright screenshots desktop + mobil (`traffic-notices-ul-*.png`) | Test | öppen |
| TF-F5 | UL-lik fixture `e2e/fixtures/traffic-notices-payload.mjs` (`panels`, Tåg+Buss, upcoming) | Test | öppen |
| TF-F6 | `traffic-demo-data.php` — texter via nya fält efter TF-A | Backend + Test | öppen |

### Fas H — Slutpolish (output på test3)

| ID | Punkt | Ansvar | Status |
|----|-------|--------|--------|
| TF-H1 | Enqueue `mrt-traffic-info-tokens.css` — mönster `brand-tokens.php` / hook i `vue-frontend.php` + shortcode | Backend | öppen |
| TF-H2 | Verifiera `alignwide` (`vue-mount-layout.php`) + `MrtPublicAppShell` på Lennakatten-tema (test3) | Test + Produkt | öppen |

### Fas G — Acceptans

| ID | Punkt | Ansvar | Status |
|----|-------|--------|--------|
| TF-G1 | Jesper-checklista §14 i planen (8 punkter, mobil + desktop) | Produkt | öppen |
| TF-G2 | Jesper OK på UL 1:1 målbild (uppföljning J11) | Produkt | öppen |

**Kritiskt för synligt resultat på test3:** TF-A + TF-F5 + TF-H1 — utan dem ser feeden «fel» även med perfekt CSS.
