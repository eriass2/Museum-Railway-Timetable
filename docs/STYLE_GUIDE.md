...
- **Enqueue** – Loader `inc/assets.php` inkluderar `inc/assets/admin.php` och `inc/assets/frontend.php` (admin: bl.a. `mrt-string-utils` före `mrt-admin-utils`; frontend: `mrt-string-utils` + `mrt-frontend-api` före `mrt-frontend`; wizard + tidtabellsöversikt även `train-type-icons.css`).
