...
/**
 * Add traffic disruptions page to primary nav menu when missing (idempotent).
 */
function MRT_append_traffic_disruptions_to_nav_menu( int $page_id ): int {
	if ( $page_id <= 0 || ! function_exists( 'MRT_get_assigned_nav_menu_id_for_theme' ) ) {
		return 0;
	}
	$menu_id = MRT_get_assigned_nav_menu_id_for_theme();
	...
	$added = 0;
	if ( function_exists( 'MRT_append_nav_menu_page_if_missing' )
		&& MRT_append_nav_menu_page_if_missing(
			$menu_id,
			$page_id,
