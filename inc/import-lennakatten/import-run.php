...
function MRT_import_create_routes($station_ids) {
    ...
    return [$route_id, $route_rev_id, $route_station_ids, $route_rev_station_ids];
}
...
function MRT_import_create_timetable() {
    ...
    $existing = get_posts(['post_type' => 'mrt_timetable', 'posts_per_page' => 1, 'meta_key' => 'mrt_timetable_type', 'meta_value' => 'green']);
    if (empty($existing)) {
        $id = wp_insert_post([
