...
import { AdminFieldStack, AdminFormActions, AdminPanel, AdminUnsavedBanner, MrtButton } from '../ui';
...
<template>
  <AdminPanel class="mrt-admin-timetable-meta">
    ...
    <AdminFieldStack :label="adminStr(cfg, 'editorTitleLabel')" label-for="mrt-tt-title">
      <input id="mrt-tt-title" v-model="editTitle" type="text" class="regular-text" />
    </AdminFieldStack>
    <AdminFieldStack
      :label="adminStr(cfg, 'editorTypeLabel')"
      label-for="mrt-tt-type"
      :description="adminStr(cfg, 'editorTypeHint')"
    >
